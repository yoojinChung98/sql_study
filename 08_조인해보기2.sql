...
FROM info
JOIN auth -- 그냥 JOIN 이라고만 쓰면 기본적으로 INNER JOIN 이 됨.
ON info.auth_id = auth.auth_id;

-- 필요한 데이터만 조회하겠다!
-- WHERE 구문을 통해 일반 조건을 걸어줄 수 있음.
SELECT
...
-- CROSS JOIN은 JOIN 조건을 설정하지 않기 때문에 단순히 모든 컬럼에 대해 JOIN을 진행
-- 데이터가 방대하고 의미가 없기 때문에 실제로는 거의 사용하지 않음.
SELECT
    *
FROM info
CROSS JOIN auth
...
-- 여러 개 테이블 조인 -> 공통적으로 갖고 있는 key 값을 찾아서 구문을 연결해서 쓰면 됨
SELECT
    *
FROM employees e
